...
const ERROR_MESSAGE = {
  COACH_NUMBER: '[ERROR] 같이 식사할 수 있는 인원수는 최소 2명 최대 5명입니다.',
  NAME_LENGTH: '[ERROR] 코치 이름은 최소 2글자, 최대 4글자여야 합니다.',
  FODDS_LENGTH: '[ERROR] 못 먹는 음식은 2개 이하여야 합니다.',
};

const CONDITION = {
  COACH_NUMBER: (names) => names.length < 2 || names.length > 5,
  NAME_LENGTH: (names) =>
    names.some((name) => name.length < 2 || name.length > 4),
